class User < ActiveRecord::Base
  ...
  validates_presence_of :email, :first_Name, :phone, :committee, :password, :last_Name, :username
  validates_uniqueness_of :username, :email
  validates_format_of :email, :with => /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, :message => "Invalid email" 
  attr_protected :id, :role
  attr_accessible :email, :committee, :first_Name, :last_Name, :password, :phone, :username, :role
